...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect)
{
    ...
    while (!interruptNet)
    {
        ...
        open_private_broadcast_to = m_private_broadcast.ShouldOpen(open_private_broadcast_to.has_value(),
                                                                   num_private_broadcast_opened);

        // Don't wait for outbound connection slot to be available if we are going
        // to open a connection of type PRIVATE_BROADCAST.
        CSemaphoreGrant grant;
        if (!open_private_broadcast_to.has_value()) {
            grant = CSemaphoreGrant{*semOutbound};
        }
...
CConnman::PrivateBroadcast::ShouldOpen(bool prev_was_private_broadcast, size_t num_opened) const
{
    ...
    LogPrintLevel(BCLog::PRIVATE_BROADCAST,
                  ...
                  num_to_open, GetNetworkName(net),
                  net == NET_IPV4 || net == NET_IPV6 ? " through the Tor proxy at " + tor_proxy.ToString() : "");
