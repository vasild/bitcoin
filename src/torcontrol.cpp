...
void TorControlConnection::readcb(struct bufferevent *bev, void *ctx)
{
        ...
        if (ch == ' ') {
            ...
            if (self->message.code >= 600) {
                ...
                if (!self->reply_handlers.empty()) {
                    ...
                    LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Received unexpected sync reply %i\n", self->message.code);
                }
            }
            self->message.Clear();
        }
    ...
    if (evbuffer_get_length(input) > MAX_LINE_LENGTH) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Disconnecting because MAX_LINE_LENGTH exceeded\n");
        self->Disconnect();
    }
}
...
void TorControlConnection::eventcb(struct bufferevent *bev, short what, void *ctx)
{
    ...
    if (what & BEV_EVENT_CONNECTED) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Successfully connected!\n");
        self->connected(*self);
    } else if (what & (BEV_EVENT_EOF|BEV_EVENT_ERROR)) {
        if (what & BEV_EVENT_ERROR) {
            LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Error connecting to Tor control socket\n");
        } else {
            LogPrintLevel(BCLog::TOR, BCLog::Level::Warning, "End of stream\n");
        }
        self->Disconnect();
...
bool TorControlConnection::Connect(const std::string& tor_control_center, const ConnectionCB& _connected, const ConnectionCB& _disconnected)
{
    ...
    if (!Lookup(tor_control_center, control_service, 9051, fNameLookup)) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Failed to look up control center %s\n", tor_control_center);
        return false;
    }
    ...
    if (!control_service.GetSockAddr(reinterpret_cast<struct sockaddr*>(&control_address), &control_address_len)) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Error parsing socket address %s\n", tor_control_center);
        return false;
    }
    ...
    if (bufferevent_socket_connect(b_conn, reinterpret_cast<struct sockaddr*>(&control_address), control_address_len) < 0) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Error connecting to address %s\n", tor_control_center);
        return false;
    }
    return true;
}
    ...
    m_target(target)
{
    ...
    if (!reconnect_ev)
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Failed to create event for reconnection: out of memory?\n");
    // Start connection attempts immediately
    if (!conn.Connect(m_tor_control_center, std::bind(&TorController::connected_cb, this, std::placeholders::_1),
         std::bind(&TorController::disconnected_cb, this, std::placeholders::_1) )) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Initiating connection to Tor control port %s failed\n", m_tor_control_center);
    }
    // Read service private key if cached
    std::pair<bool,std::string> pkf = ReadBinaryFile(GetPrivateKeyFile());
    if (pkf.first) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Reading cached private key from %s\n", fs::PathToString(GetPrivateKeyFile()));
...
void TorController::get_socks_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    ...
    if (reply.code == 250) {
        ...
        if (!socks_location.empty()) {
            LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Get SOCKS port command yielded %s\n", socks_location);
        } else {
            LogPrintLevel(BCLog::TOR, BCLog::Level::Warning, "Get SOCKS port command returned nothing\n");
        }
    } else if (reply.code == 510) {  // 510 Unrecognized command
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Get SOCKS port command failed with unrecognized command (you probably should upgrade Tor)\n");
    } else {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Get SOCKS port command failed; error code %d\n", reply.code);
    }
    ...
    Assume(resolved.IsValid());
    LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Configuring onion proxy for %s\n", resolved.ToStringIPPort());
...
{
    if (reply.code == 250) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "ADD_ONION successful\n");
        ...
        if (service_id.empty()) {
            LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Error parsing ADD_ONION parameters:\n");
            for (const std::string &s : reply.lines) {
                LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "    %s\n", SanitizeString(s));
            }
            return;
        }
        service = LookupNumeric(std::string(service_id+".onion"), Params().GetDefaultPort());
        LogPrintLevel(BCLog::TOR, BCLog::Level::Info, "Got service ID %s, advertising service %s\n", service_id, service.ToString());
        if (WriteBinaryFile(GetPrivateKeyFile(), private_key)) {
            LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Cached service private key to %s\n", fs::PathToString(GetPrivateKeyFile()));
        } else {
            LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Error writing service private key to %s\n", fs::PathToString(GetPrivateKeyFile()));
        }
        AddLocal(service, LOCAL_MANUAL);
        // ... onion requested - keep connection open
    } else if (reply.code == 510) { // 510 Unrecognized command
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Add onion failed with unrecognized command (you probably need to upgrade Tor)\n");
    } else {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Add onion failed; error code %d\n", reply.code);
    }
}
...
{
    if (reply.code == 250) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Authentication successful\n");
        ...
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Authentication failed\n");
    }
}
...
{
    if (reply.code == 250) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "SAFECOOKIE authentication challenge successful\n");
        std::pair<std::string,std::string> l = SplitTorReplyLine(reply.lines[0]);
        if (l.first == "AUTHCHALLENGE") {
            std::map<std::string,std::string> m = ParseTorReplyMapping(l.second);
            if (m.empty()) {
                LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Error parsing AUTHCHALLENGE parameters: %s\n", SanitizeString(l.second));
                return;
            }
            std::vector<uint8_t> serverHash = ParseHex(m["SERVERHASH"]);
            std::vector<uint8_t> serverNonce = ParseHex(m["SERVERNONCE"]);
            LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "AUTHCHALLENGE ServerHash %s ServerNonce %s\n", HexStr(serverHash), HexStr(serverNonce));
            if (serverNonce.size() != 32) {
                LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "ServerNonce is not 32 bytes, as required by spec\n");
                return;
            }

            std::vector<uint8_t> computedServerHash = ComputeResponse(TOR_SAFE_SERVERKEY, cookie, clientNonce, serverNonce);
            if (computedServerHash != serverHash) {
                LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "ServerHash %s does not match expected ServerHash %s\n", HexStr(serverHash), HexStr(computedServerHash));
                return;
            }
            ...
            LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Invalid reply to AUTHCHALLENGE\n");
        }
    } else {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "SAFECOOKIE authentication challenge failed\n");
    }
}
    ...
    if (reply.code == 250) {
        ...
        for (const std::string &s : reply.lines) {
            ...
            if (l.first == "AUTH") {
                ...
                if ((i = m.find("Tor")) != m.end()) {
                    LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Connected to Tor version %s\n", i->second);
                }
            }
        }
        for (const std::string &s : methods) {
            LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Supported authentication method: %s\n", s);
        }
        ...
        if (!torpassword.empty()) {
            if (methods.count("HASHEDPASSWORD")) {
                LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Using HASHEDPASSWORD authentication\n");
                ReplaceAll(torpassword, "\"", "\\\"");
                _conn.Command("AUTHENTICATE \"" + torpassword + "\"", std::bind(&TorController::auth_cb, this, std::placeholders::_1, std::placeholders::_2));
            } else {
                LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Password provided with -torpassword, but HASHEDPASSWORD authentication is not available\n");
            }
        } else if (methods.count("NULL")) {
            LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Using NULL authentication\n");
            _conn.Command("AUTHENTICATE", std::bind(&TorController::auth_cb, this, std::placeholders::_1, std::placeholders::_2));
        } else if (methods.count("SAFECOOKIE")) {
            // Cookie: hexdump -e '32/1 "%02x""\n"'  ~/.tor/control_auth_cookie
            LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Using SAFECOOKIE authentication, reading cookie authentication from %s\n", cookiefile);
            ...
            if (status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE) {
                ...
                if (status_cookie.first) {
                    LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Authentication cookie %s is not exactly %i bytes, as is required by the spec\n", cookiefile, TOR_COOKIE_SIZE);
                } else {
                    LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Authentication cookie %s could not be opened (check permissions)\n", cookiefile);
                }
            }
        } else if (methods.count("HASHEDPASSWORD")) {
            LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "The only supported authentication mechanism left is password, but no password provided with -torpassword\n");
        } else {
            LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "No supported authentication method\n");
        }
    } else {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Requesting protocol info failed\n");
    }
}
...
void TorController::connected_cb(TorControlConnection& _conn)
{
    ...
    if (!_conn.Command("PROTOCOLINFO 1", std::bind(&TorController::protocolinfo_cb, this, std::placeholders::_1, std::placeholders::_2)))
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Error sending initial protocolinfo command\n");
}
...
void TorController::disconnected_cb(TorControlConnection& _conn)
{
    ...
    LogPrintLevel(BCLog::TOR, BCLog::Level::Debug, "Not connected to Tor control port %s, trying to reconnect\n", m_tor_control_center);
...
void TorController::Reconnect()
{
    ...
    if (!conn.Connect(m_tor_control_center, std::bind(&TorController::connected_cb, this, std::placeholders::_1),
         std::bind(&TorController::disconnected_cb, this, std::placeholders::_1) )) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Re-initiating connection to Tor control port %s failed\n", m_tor_control_center);
    }
}
...
void StartTorControl(CService onion_service_target)
{
    ...
    if (!gBase) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Unable to create event_base\n");
        return;
    }
...
{
    if (gBase) {
        LogPrintLevel(BCLog::TOR, BCLog::Level::Error, "Thread interrupt\n");
