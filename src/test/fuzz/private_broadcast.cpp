    ...
    while (fdp.remaining_bytes() > 0) {
        CallOneOf(
            ...
            [&] { // PushedToNode()
                ...
                const auto it{PickIterator(fdp, transactions)};
                const auto& tx{it->first};
                auto& pushed_to_nodes{it->second};
                pb.PushedToNode(new_nodeid, tx->GetHash());
                pushed_to_nodes.emplace(new_nodeid);
            },
            [&] { // GetTxPushedToNode()
                if (transactions.empty()) {
                    return;
                }
                const auto it_transactions{PickIterator(fdp, transactions)};
                const auto& pushed_tx{it_transactions->first};
                const auto& pushed_to_nodes{it_transactions->second};
            ...
            [&] { // FinishBroadcast()
                ...
                auto& pushed_to_nodes{PickIterator(fdp, transactions)->second};
                if (pushed_to_nodes.empty()) {
                    return;
                }
