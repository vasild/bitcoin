                           ...
                           std::string& errmsg) const
{
    ...
    std::shared_ptr<NodeSockets> node_sockets;

    {
        ...
        if (it == m_connected.end()) {
            ...
            // in the caller's send queue.
            return 0;
        }
        node_sockets = it->second;
    }
...
void SockMan::ThreadSocketHandler()
{
    ...
    while (!interruptNet) {
        ...
        // empty sets.
        events_per_sock = GenerateWaitSockets();
        if (events_per_sock.empty() ||
            !events_per_sock.begin()->first->WaitMany(SELECT_TIMEOUT, events_per_sock)) {
...
Sock::EventsPerSock SockMan::GenerateWaitSockets()
{
    ...
    for (const auto& [node_id, node_sockets] : connected_snapshot) {
        ...
        events_per_sock.emplace(node_sockets->sock, Sock::Events{event});
    }

    return events_per_sock;
}

void SockMan::SocketHandlerConnected(const Sock::EventsPerSock& events_per_sock)
{
    AssertLockNotHeld(m_connected_mutex);
    ...
    for (const auto& [node_id, node_sockets] : connected_snapshot) {
        ...
        bool err_ready{false};

        const auto it{events_per_sock.find(node_sockets->sock)};
        if (it != events_per_sock.end()) {
            recv_ready = it->second.occurred & Sock::RECV;
            send_ready = it->second.occurred & Sock::SEND;
            err_ready = it->second.occurred & Sock::ERR;
        }
